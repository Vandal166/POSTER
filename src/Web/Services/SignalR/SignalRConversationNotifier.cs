...
internal sealed class SignalRConversationNotifier : IConversationNotifier
{
    ...
    public async Task NotifyParticipantAddedAsync(Guid conversationID, Guid participantID, CancellationToken ct = default)
    {
        await _hubContext.Clients.Group(conversationID.ToString())
            .SendAsync("ParticipantAdded", conversationID, participantID, ct);
    }
