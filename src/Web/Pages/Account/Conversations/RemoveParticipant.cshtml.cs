...
public class RemoveParticipant : PageModel
{
    ...
    public RemoveParticipant(ICurrentUserService currentUser, IConversationService conversationService, IToastBuilder toastBuilder, IConversationMessageService conversationMessageService)
    {
    ...
    public async Task<IActionResult> OnPostAsync(Guid conversationId, Guid participantId, string participantName, CancellationToken ct = default)
    {
        ...
        }//TODO try XSS into this ^^, the admin can set the name of the participant via dev tools to smth like: <script>alert(1)</script>
