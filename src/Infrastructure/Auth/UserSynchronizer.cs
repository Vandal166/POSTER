...
public class UserSynchronizer : IUserSynchronizer
{
    ...
    public async Task SyncAsync(Guid keycloakID, CancellationToken ct = default)
    {
        var kcUser = await _kc.GetUserAsync(keycloakID, ct)
                     ?? throw new InvalidOperationException("Keycloak user not found.");
        

        var existing = await _users.GetUserAsync(keycloakID, ct);
        if (existing is null)
        {
            var user = User.Create(keycloakID, kcUser.Username).Value;
